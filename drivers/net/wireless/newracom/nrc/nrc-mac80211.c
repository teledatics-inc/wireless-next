...
/**
 * nrc_wake_tx_queue - move pending frames into the driver's pending list
 * @hw: hardware context owning @txq
 * @txq: queue whose frames should be scheduled
 *
 * The function moves skb in @txq to sk_buff_head in its private data
 * structure, and adds it to the list of queues awaiting service.
 */
static
...
static int nrc_mac_config(struct ieee80211_hw *hw, int radio_idx, u32 changed)
{
	...
	if (hw)
		memcpy(&chandef, &hw->conf.chandef,
		       sizeof(struct cfg80211_chan_def));
	if (hw && hw->conf.chandef.chan) {
		memcpy(&ch, hw->conf.chandef.chan,
...
static struct timer_list remotecmd_timer;
static struct remotecmd_params remotecmd_params;
...
static VCMD_BACKUP_INFO vcmd_backup_info[VIF_MAX];
