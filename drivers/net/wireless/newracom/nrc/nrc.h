...
#define STAH(fn)					\
	extern struct nrc_sta_handler __stah_ ## fn; \
	struct nrc_sta_handler __stah_ ## fn __attribute__((__used__)) = {	\
		.sta_state = fn,			\
	}
...
#define TXH(fn, mask)					\
	extern struct nrc_trx_handler __txh_ ## fn; \
	struct nrc_trx_handler __txh_ ## fn __attribute__((__used__)) = {	\
		.handler = fn,				\
		.vif_types = mask,			\
	}

#define RXH(fn, mask)					\
	extern struct nrc_trx_handler __rxh_ ## fn; \
	struct nrc_trx_handler __rxh_ ## fn __attribute__((__used__)) = {	\
		.handler = fn,				\
		.vif_types = mask,			\
	}
...
struct nrc_radiotap_hdr {
	...
	__le64 rt_tsft;		/* IEEE80211_RADIOTAP_TSFT */
	u8 rt_flags;		/* IEEE80211_RADIOTAP_FLAGS */
	u8 rt_pad;		/* pad for IEEE80211_RADIOTAP_CHANNEL */
	__le16 rt_ch_frequency;	/* IEEE80211_RADIOTAP_CHANNEL */
	__le16 rt_ch_flags;
	__le16 rt_pad2;		/* pad for IEEE80211_RADIOTAP_TLV:S1G */
	__le16 rt_tlv_type;	/* IEEE80211_RADIOTAP_TLV */
	__le16 rt_tlv_length;
	__le16 rt_s1g_known;
	__le16 rt_s1g_data1;
	__le16 rt_s1g_data2;
} __packed;
...
struct nrc_radiotap_hdr_agg {
	...
	__le16 rt_pad2;
	__le32 rt_ampdu_ref;	/* IEEE80211_RADIOTAP_AMPDU_STATUS */
	__le16 rt_ampdu_flags;
	u8 rt_ampdu_crc;
	u8 rt_ampdu_reserved;
	__le16 rt_tlv_type;	/* IEEE80211_RADIOTAP_TLV */
	__le16 rt_tlv_length;
	__le16 rt_s1g_known;
	__le16 rt_s1g_data1;
...
struct nrc_radiotap_hdr_ndp {
	...
	__le16 rt_ch_frequency;	/* IEEE80211_RADIOTAP_CHANNEL */
	__le16 rt_ch_flags;
	u8 rt_rssi;		/* IEEE80211_RADIOTAP_DBM_ANTSIGNAL */
	u8 rt_zero_length_psdu; /* IEEE80211_RADIOTAP_0_LENGTH_PSDU */
} __packed;
