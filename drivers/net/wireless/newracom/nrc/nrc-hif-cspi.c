...
static int spi_read_sys_reg (struct spi_device *spi, struct spi_sys_reg *sys)
{
	...
	sys->chip_id = be16_to_cpu((__force __be16)sys->chip_id);
	sys->modem_id = be32_to_cpu((__force __be32)sys->modem_id);
	sys->sw_id = be32_to_cpu((__force __be32)sys->sw_id);
	sys->board_id = be32_to_cpu((__force __be32)sys->board_id);
...
static int spi_update_status(struct spi_device *spi)
{
	...
	msg0 = be32_to_cpu(status->msg[0]);
	msg1 = be32_to_cpu(status->msg[1]);
	msg2 = be32_to_cpu(status->msg[2]);
	msg3 = be32_to_cpu(status->msg[3]);
...
update:
	...
	priv->slot[RX_SLOT].head = msg0 & 0xffff;
	priv->slot[TX_SLOT].head = msg0 >> 16;
	...
	if (c_spi_num_slots(priv, TX_SLOT) > 32) {
		...
		if (priv->hw.sys.chip_id == 0x7394 && nw->drv_state == NRC_DRV_PS &&
				((msg3 & 0xffff) == TARGET_NOTI_REQUEST_FW_DOWNLOAD)) {
...
static int spi_start(struct nrc_hif_device *dev)
{
	...
	priv->slot[TX_SLOT].tail = be32_to_cpu(status->msg[3]) & 0xffff;
...
static void spi_update(struct nrc_hif_device *hdev)
{
	...
	priv->slot[TX_SLOT].count = status->rxq_status[1] & RXQ_SLOT_COUNT;
	priv->slot[RX_SLOT].count = status->txq_status[1] & TXQ_SLOT_COUNT;
	priv->slot[RX_SLOT].head = be32_to_cpu(status->msg[0]) & 0xffff;
	priv->slot[TX_SLOT].head = be32_to_cpu(status->msg[0]) >> 16;

	trace_nrc_hif_rx_slot(priv, RX_SLOT, "spi update");
	trace_nrc_hif_tx_slot(priv, TX_SLOT, "spi update");

	/* Update VIF0 credit */
	rear = be32_to_cpu(status->msg[1]);
	for (ac = 0; ac < 4; ac++)
		priv->rear[ac] = (rear >> 8*ac) & 0xff;

	/* Update VIF1 credit */
	rear = be32_to_cpu(status->msg[2]);
	for (ac = 0; ac < 4; ac++)
		priv->rear[6+ac] = (rear >> 8*ac) & 0xff;
}
