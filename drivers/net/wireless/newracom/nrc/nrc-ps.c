...
#define nrc_try_to_del_timer_sync(timer) timer_delete_sync_try(timer)
#else
#define nrc_try_to_del_timer_sync(timer) try_to_del_timer_sync(timer)
#endif
...
int nrc_ps_set_mode (struct nrc *nw, enum NRC_PS_MODE mode, int timeout)
{
	...
	if (nw->drv_state == NRC_DRV_CLOSING)
		goto done;
	if (!disable_cqm) {
		nrc_try_to_del_timer_sync(&nw->bcn_mon_timer);
	}
