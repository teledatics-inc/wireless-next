...
DECLARE_EVENT_CLASS(nrc_hdr_event,
	...
	TP_fast_assign(
		...
		u16 fc_host = le16_to_cpu(fc);
		__entry->fc_type = fc_host & IEEE80211_FCTL_FTYPE;
		__entry->fc_subtype = fc_host & IEEE80211_FCTL_STYPE;
		__entry->fc_etc = fc_host & 0xFF00;
		memcpy(__entry->addr1, hdr->addr1, ETH_ALEN);
		memcpy(__entry->addr2, hdr->addr2, ETH_ALEN);
		memcpy(__entry->addr3, hdr->addr3, ETH_ALEN);
		__entry->sn = IEEE80211_SEQ_TO_SN(le16_to_cpu(hdr->seq_ctrl));
	...
	TP_printk(
		...
		//__get_str(device),
		__entry->len,
	(__entry->fc_type == IEEE80211_FTYPE_DATA)?__print_flags(__entry->fc_subtype, " ",
		...
		):(__entry->fc_type == IEEE80211_FTYPE_MGMT)?get_mgmt_str(__entry->fc_subtype):"???",
