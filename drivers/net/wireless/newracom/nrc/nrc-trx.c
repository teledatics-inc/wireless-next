...
extern struct nrc_trx_handler __txh_tx_h_debug_print;
#endif
extern struct nrc_trx_handler __txh_tx_h_debug_state;
extern struct nrc_trx_handler __txh_tx_h_wfa_halow_filter;
extern struct nrc_trx_handler __txh_tx_h_frame_filter;
extern struct nrc_trx_handler __txh_tx_h_managed_p2p_intf_addr;
extern struct nrc_trx_handler __txh_tx_h_put_iv;
extern struct nrc_trx_handler __txh_tx_h_put_qos_control;
extern struct nrc_trx_handler __txh_tx_h_twt_assoc;
extern struct nrc_trx_handler __txh_tx_h_sta_pm;
extern struct nrc_trx_handler __txh_tx_h_bss_max_idle_period;

extern struct nrc_trx_handler __rxh_rx_h_vendor;
extern struct nrc_trx_handler __rxh_rx_h_decrypt;
extern struct nrc_trx_handler __rxh_rx_h_check_sn;
#if defined(CONFIG_SUPPORT_IBSS)
extern struct nrc_trx_handler __rxh_rx_h_ibss_get_bssid_tsf;
#endif
extern struct nrc_trx_handler __rxh_rx_h_action;
extern struct nrc_trx_handler __rxh_rx_h_twt_assoc;
extern struct nrc_trx_handler __rxh_rx_h_twt_monitor;
#if NRC_DBG_PRINT_FRAME_RX
extern struct nrc_trx_handler __rxh_rx_h_debug_print;
#endif
#ifdef CONFIG_SUPPORT_MESH_ROUTING
extern struct nrc_trx_handler __rxh_rx_h_mesh;
#endif
extern struct nrc_trx_handler __rxh_rx_h_fixup_ps_poll_sp;
extern struct nrc_trx_handler __rxh_rx_h_bss_max_idle_period;

extern struct nrc_sta_handler __stah_sta_h_bss_max_idle_period;

struct nrc_trx_handler * const nrc_tx_handlers[] = {
#if NRC_DBG_PRINT_FRAME_TX
	&__txh_tx_h_debug_print,
#endif
	&__txh_tx_h_debug_state,
	&__txh_tx_h_wfa_halow_filter,
	&__txh_tx_h_frame_filter,
	&__txh_tx_h_managed_p2p_intf_addr,
	&__txh_tx_h_put_iv,
	&__txh_tx_h_put_qos_control,
	&__txh_tx_h_twt_assoc,
	&__txh_tx_h_sta_pm,
	&__txh_tx_h_bss_max_idle_period,
};

const size_t nrc_tx_handlers_count = ARRAY_SIZE(nrc_tx_handlers);

struct nrc_trx_handler * const nrc_rx_handlers[] = {
	&__rxh_rx_h_vendor,
	&__rxh_rx_h_decrypt,
	&__rxh_rx_h_check_sn,
#if defined(CONFIG_SUPPORT_IBSS)
	&__rxh_rx_h_ibss_get_bssid_tsf,
#endif
	&__rxh_rx_h_action,
	&__rxh_rx_h_twt_assoc,
	&__rxh_rx_h_twt_monitor,
#if NRC_DBG_PRINT_FRAME_RX
	&__rxh_rx_h_debug_print,
#endif
#ifdef CONFIG_SUPPORT_MESH_ROUTING
	&__rxh_rx_h_mesh,
#endif
	&__rxh_rx_h_fixup_ps_poll_sp,
	&__rxh_rx_h_bss_max_idle_period,
};

const size_t nrc_rx_handlers_count = ARRAY_SIZE(nrc_rx_handlers);

struct nrc_sta_handler * const nrc_sta_handlers[] = {
	&__stah_sta_h_bss_max_idle_period,
};

const size_t nrc_sta_handlers_count = ARRAY_SIZE(nrc_sta_handlers);
				    ...
				    unsigned int hdr_len)
{
	...
	__le16 fc = 0;
	__le16 qos_ctrl = 0; /* Set TID '0' when converting to QoS data. */
...
static void nrc_mac_rx_h_status(struct nrc *nw, struct sk_buff *skb)
{
	...
	if (ieee80211_has_order(mh->frame_control))
	...
	if (!rx.result) {
		...
		if ((ieee80211_is_assoc_req(mh->frame_control) ||
			...
			i_sta->listen_interval =
				le16_to_cpu(mgmt->u.assoc_req.listen_interval);
		}
...
nrc_add_rx_s1g_radiotap_header(struct nrc *nw, struct sk_buff *skb)
{
	...
	if (ndp_ind) {
		...
		rt_hdr_ndp.hdr.it_len = cpu_to_le16(sizeof(rt_hdr_ndp));
	...
	rt_hdr_agg.hdr.it_len = cpu_to_le16(sizeof(rt_hdr_agg));
	...
	rt_hdr_agg.rt_ampdu_ref = cpu_to_le32(nrc_ampdu_mon_get_ref_id(nw, skb));
	rt_hdr_agg.rt_ampdu_flags = cpu_to_le16(2);
	...
	rt_hdr_agg.rt_s1g_known  = cpu_to_le16(0x007F);
	rt_hdr_agg.rt_s1g_data1  = cpu_to_le16(
		((rxi->bandwidth == 0) ? 0x0 :
	...
	rt_hdr.hdr.it_len = cpu_to_le16(sizeof(rt_hdr));
	...
	rt_hdr.rt_s1g_known		= cpu_to_le16(0x007F);
	rt_hdr.rt_s1g_data1		= cpu_to_le16(
		((rxi->bandwidth == 0) ? 0x0 :
			((rxi->preamble_type) ? 0x2 : 0x1)) |
		((response_ind & 0x03) << 2) |	 /* RI */
		((short_gi & 0x01) << 5) |		 /* GI */
