				    ...
				    struct ieee80211_sta *sta)
{
	...
	skb = ieee80211_deauth_get(hw, sta->addr, vif->addr, vif->addr,
				   cpu_to_le16(WLAN_REASON_DISASSOC_DUE_TO_INACTIVITY),
				   sta, true);
	if (!skb)
		return -1;
...
#define NRC_TIMER_FROM(var, timer, member) \
	timer_container_of(var, timer, member)
#define nrc_del_timer_sync(timer) \
...
{
	struct nrc_vif *i_vif = NRC_TIMER_FROM(i_vif, t, max_idle_timer);
#endif
...
void ap_max_idle_timer_stop (struct nrc *nw, struct nrc_vif *i_vif)
{
	if(timer_pending(&i_vif->max_idle_timer)){
		...
		nrc_del_timer_sync(&i_vif->max_idle_timer);
	}
}
...
static int tx_h_bss_max_idle_period(struct nrc_trx_data *tx)
{
	...
	ie->max_idle_period = cpu_to_le16(i_sta->max_idle.period);
 ...
 out:
	nrc_mac_dbg("%s: %s, max_idle_period(16bit)=%u", __func__,
		    ...
		    le16_to_cpu(ie->max_idle_period));
	...
	if (ie) {
		if (rx->vif->type == NL80211_IFTYPE_AP) {
			...
			if (ie->max_idle_period != 0) {
				i_sta->max_idle.period = le16_to_cpu(ie->max_idle_period);
